...
>
> 状态：`WEB_REWORK_COMMITTED / PRODUCT_READONLY_REVIEW`
...
交接文件最初要求完成后停在产品只读 Review，不得自行提交或推送；用户随后明确授权提交本次九项 Web 返修。因此：

- 本轮代码、直接测试和本交付文件进入同一协作提交；提交 SHA 以提交后的回传消息为准，避免文档自引用。
- 已获授权提交；未推送、未合并、未创建 PR、未打标签、未冻结、未部署。
...
3. 使用回传的新提交 SHA 执行产品只读 Review；需要远端交付时，再由用户明确授权推送协作分支。
