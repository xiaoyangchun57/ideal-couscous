...
test('monitoring merge preserves the legacy site status and retains last success on refresh failure', () => {
  ...
  const first = mergeMonitoringSites(legacy, {
    items: [{ id: 7, monitoring_status: 'normal', monitoring_reason: '有效观测在周期内',
      ...
      latest_values: [{ business_metric: 'ph', factor_name_cn: '酸碱度', standard_value: 0, standard_unit: 'pH' }] }],
  });
  assert.equal(first[0].status, 'maintenance');
  assert.equal(first[0].monitoring_status, 'normal');
  assert.equal(first[0].latest_values[0].standard_value, 0);
  ...
  assert.equal(retained[0].last_received_at, '2026-09-15T08:00:00+08:00');
  assert.equal(retained[0].last_valid_observation_at, '2026-09-10T07:00:00+08:00');
  assert.equal(retained[0].latest_values[0].standard_value, 0);
});
...
test('a successful response missing a site cannot reuse its previous monitoring facts', () => {
  ...
  const previous = [{ id: 7, monitoring_status: 'normal', monitoring_reason: '旧原因',
    last_received_at: '2026-09-15T08:00:00+08:00', last_valid_observation_at: '2026-09-10T07:00:00+08:00',
    latest_values: [{ standard_value: 0 }] }];
  ...
  assert.equal(next[0].last_received_at, undefined);
  assert.equal(next[0].last_valid_observation_at, undefined);
  assert.equal(next[0].latest_values, undefined);
});
