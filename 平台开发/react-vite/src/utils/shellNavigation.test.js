...
test('notification targets respect role-visible pages', () => {
  ...
  assert.equal(getNotificationTarget({ source_type: 'spare_part_request', source_id: 9 }, ['admin']), '/audit?tab=parts&request=9');
...
test('audit notifications locate every supported review object', () => {
  ...
  assert.equal(getNotificationTarget({ source_type: 'parts_request', source_id: 15 }, ['admin']), '/audit?tab=parts&request=15');
